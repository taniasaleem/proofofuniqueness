...
class MasterNode extends Node {
  ...
  private tokenHashes: Map<string, { hash: string; timestamp: number; verifiedBy: string[] }> = new Map();
  ...
  private registerTokenHash(serialNumber: string, hash: string) {
    // Store the token hash locally
    this.tokenHashes.set(serialNumber, {
      hash,
      timestamp: Date.now(),
      verifiedBy: []
  ...
  getTokenHash(serialNumber: string) {
    return this.tokenHashes.get(serialNumber);
  }

  getVerificationCount(serialNumber: string): number {
    const tokenData = this.tokenHashes.get(serialNumber);
    return tokenData ? tokenData.verifiedBy.length : 0;
  }
}
