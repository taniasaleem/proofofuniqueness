...
export default function NodeManager() {
        ...
        switch (message.type) {
          ...
          case "error":
            ...
            if (message.data?.message === "Token hash not found") {
              console.log(
                "Token hash not found - this is expected for new nodes"
              );
              setStatus("Waiting for token hash from master node...");
              // If we have a serial number, request the token hash again
              if (serialNumber) {
                console.log(
                  "Requesting token hash again for serial number:",
                  serialNumber
                );
                setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (savedNode) {
      ...
      if (nodeInfo.serialNumber && isConnected) {
        console.log(
          "Requesting token hash for saved node:",
          nodeInfo.serialNumber
        );
  ...
  useEffect(() => {
    ...
    if (isNodeCreated && isConnected && serialNumber) {
      console.log(
        "Setting up token hash fetch interval with serial number:",
        serialNumber
      );

      // Initial call
      console.log(
        "Making initial getTokenHash call with serial number:",
        serialNumber
      );
      getTokenHash(serialNumber);
  ...
  const createNode = () => {
    try {
      ...
      console.log(
        "Creating new node with serial number:",
        newNode.serialNumber
      );
      ...
      setIsNodeCreated(true);
      setStatus(
        "Node created successfully. Waiting for token hash from master node..."
      );

      // Request token hash immediately after node creation
      if (nodeInfo.serialNumber) {
        console.log(
          "Requesting token hash for new node with serial number:",
          nodeInfo.serialNumber
        );
        getTokenHash(nodeInfo.serialNumber);
      }
      ...
      <section id="mynode_overview">
        <div className="content_ctr nodeinfo">
          ...
          <p className="info_ctr">
            Node Serial # <span>{serialNumber}</span>
          </p>
          ...
          <p className="info_ctr">
            ...
            <span>
              {tokenHash.length > 20
                ? `${tokenHash.slice(0, 10)}...${tokenHash.slice(-10)}`
                : tokenHash}
            </span>
          </p>

          <p className="info_ctr">
            Node Size <span>Small</span>
          </p>

          <p className="info_ctr">
            Node IP <span>d5a1310ae653</span>
          </p>

          <p className="info_ctr">
            Status <span className="status">Started</span>
          </p>
          ...
          <p className="info_ctr">
            Region <span>AWS: US-east-2</span>
          </p>

          <p className="info_ctr">
            Creation date <span>{connectionTime}</span>
          </p>
